(ns flybot.ports.ubergraph.core
  (:require [flybot.ports.potemkin :refer [import-vars def-map-type deftype+]]
            [flybot.ports.potemkin.collections :refer [AbstractMap]]
            [flybot.ports.loom.graph :as lg]
            [flybot.ports.loom.attr :as la]
            [flybot.ports.ubergraph.protocols :as up]
            [flybot.ports.dorothy.core :as d]
            [flybot.ports.dorothy.clr :as dclr]
...
(import-vars
 ...
 [flybot.ports.ubergraph.protocols
...
(def-map-type Ubergraph [node-map allow-parallel? undirected? attrs cached-hash]
  ...
  #?(:cljr (clojure.lang.IPersistentMap.assoc [this key value]
             (let [^clojure.lang.Associative this this]
               (assoc this key value))))
  
  (#?(:clj assoc
      :cljr clojure.lang.Associative.assoc) [this key value]
    (case key
  ...
  ([g {src :src dest :dest :as attributes}]
    (let [edges
      ...
      (if (pos? (count attributes))
        ;; (into [] ;; not lazy, which could be considered breaking. and makes find-edge-impl need work
        ;;   (filter #(submap? attributes (-> g (get :attrs) (get (:id %)))))
        ;;   edges)
        ;; (eduction
        ;;   (filter #(submap? attributes (-> g (get :attrs) (get (:id %)))))
        ;;   edges)
...
(defn- add-directed-edge [g src dest attributes]
  (let [g (-> g (add-node src) (add-node dest))
        edge-id #?(:clj (java.util.UUID/randomUUID)
...
(defn- add-undirected-edge [g src dest attributes]
  (let [g (-> g (add-node src) (add-node dest))
        forward-edge-id #?(:clj (java.util.UUID/randomUUID)
                           :cljr (System.Guid/NewGuid))
...
an edge object."
  ...
  (cond
    (edge? ed) ed
    (not (vector? ed)) (throw (#?(:clj IllegalArgumentException.
  ...
  (cond (edge? node-or-edge) (:id node-or-edge)
        ...
        (try (:id (edge-description->edge g node-or-edge))
             (catch #?(:clj IllegalArgumentException
                       :cljr ArgumentException) e
               (throw (#?(:clj IllegalArgumentException.
                          :cljr ArgumentException.) (str "Invalid node or edge description: " node-or-edge)))))))
...
(defmethod print-dup flybot.ports.ubergraph.core.Ubergraph [o w]
...
(defn- node-set [^Ubergraph g]
  ;; TODO: optimize this -tsg
  #?(:clj (let [^java.util.Map m (:node-map g)]
            (.keySet m))
     :cljr (set (keys (:node-map g)))))
